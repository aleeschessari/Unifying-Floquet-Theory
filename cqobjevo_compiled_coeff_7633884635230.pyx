...
cdef class CompiledStrCoeff(StrCoeff):
    ...
    cdef void _call_core(self, double t, complex * coeff):
        ...
        coeff[10] = (exp(-1j*w_d*t)) * (conj(exp(-1j*w_d*t)))
        coeff[11] = (conj(exp(-1j*w_d*t))) * (exp(-1j*w_d*t))
        coeff[12] = (conj(exp(-1j*w_d*t))) * (exp(-1j*w_d*t))
        coeff[13] = (exp(1j*w_d*t)) * (conj(exp(1j*w_d*t)))
        coeff[14] = (conj(exp(1j*w_d*t))) * (exp(1j*w_d*t))
        coeff[15] = (conj(exp(1j*w_d*t))) * (exp(1j*w_d*t))
