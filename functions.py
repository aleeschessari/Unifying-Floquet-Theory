    ...
    for idx,u in enumerate(u_list):
        ...
        for i,temp_eigv in enumerate(temp):
            ...
            for j,eigv in enumerate(ekets):
                if abs((temp_eigv.dag() * eigv )[0][0][0]) > weight:
                    weight = abs((temp_eigv.dag() * eigv )[0][0][0])
...
def get_derivatives(N_rep,A_q,H_sys,drive_op,wd_list,n_states,num_A):
    ...
    dd_real = np.zeros((len(wd_list),num_A,n_states))   
    dd2_real = np.zeros((len(wd_list),num_A,n_states))   
    ...
    for i,w_d in enumerate(wd_list):
        ...
        for j in range(np.shape(Z)[2]):
            dd_real[i,:,j] = np.gradient(Z[i,:,j],Aq_list)
            dd2_real[i,:,j] = np.gradient(dd_real[i,:,j],Aq_list)
        ...
        if N_replica_space > 1:
            for j in range(N_replica_space):
                for k in range(len(ref)):
                    weight[i,k] += np.array((tensor(basis(N_replica_space,j),ref[k]).dag()*eigv)[0][0][0]) # how much i-th eigv is in ref k
        else:
            for k in range(len(ref)):
                weight[i,k] += np.array(ref[k].dag()*eigv)[0] # how much i-th eigv is in ref k
