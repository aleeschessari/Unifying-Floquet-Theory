...
def construct_eigenstates_nofloquet_noqutip(H_sys, n_states):
    ...
    evals, ekets = sp.linalg.eigh(np.array(H_sys.full()))
...
def construct_eigenvalues_eigenstates_floquet_list(N_rep, last_A, num_A,
    ...
    sp_floquet = np.diag(np.ones(N_replica_space-1), 1)
    sm_floquet = np.diag(np.ones(N_replica_space-1), -1)
    ...
    tensor_0 = np.kron(np.identity(N_replica_space), np.array(H_sys.full()))
    tensor_1 = np.kron(sp_floquet, np.array(drive_op.full()))
    tensor_2 = np.kron(sm_floquet, np.array(drive_op.full()))
    tensor_3 = np.kron(m, np.diag(np.ones(n_states)))
    ...
    evals_list = np.zeros((len(u_list), n_states)) # Matrix that stores A vs E (given all the bands)

    evals, evecs = construct_eigenstates_nofloquet_noqutip(H_sys, n_states)
    temp = np.kron(diag_replica(N_rep,target_replica), np.transpose(np.conjugate(evecs)))
    #evecs_list.append(np.conjugate(np.transpose(temp)))
    #evals_list[0, :] = evals

    proj_last_replica = np.kron(np.diag(diag_replica(N_rep,N_rep)), np.identity(n_states))\
                            +np.kron(np.diag(diag_replica(N_rep,-N_rep)), np.identity(n_states))
    
    if n_states > 5:
        proj_last_states = np.kron(np.identity(N_replica_space), np.diag(np.concatenate((np.zeros(n_states-4),np.ones(4)))))

    for idx, u in enumerate(u_list):
        H_qubit = H0 + u*H1
        evals, ekets = sp.linalg.eigh(H_qubit)

        #partial_weight = np.einsum('ij,ik->jk', np.conjugate(ekets), temp)
        partial_weight = temp @ ekets # ekets has dim N_replica_space * N_replica_space. Eigenvectors are on the columns

        weight = partial_weight*np.conjugate(partial_weight)                
        i_max = np.argmax(weight, axis=1)
        evals_list[idx, :] = evals[i_max] 

        temp = np.transpose(np.conjugate(ekets[:,i_max]))
        evecs_list.append(ekets[:,i_max])

        if w_d > 0:
            overflow_replica = np.linalg.norm(proj_last_replica @ ekets[:,i_max], axis=0)
        ...
        if n_states > 5:
            overflow_states = np.linalg.norm(proj_last_states @ ekets[:,i_max], axis=0)
...
def get_derivatives(N_rep,A_q,H_sys,drive_op,wd_list,n_states,num_A,fname=None,z0_comput=None,qubit_state_list=None):
    ...
    if isinstance(A_q, np.ndarray):
        ...
        z0 = np.zeros((len(wd_list),len(A_q),2))
    ...
    if z0_comput != None:
        return Aq_list, dd_real, dd2_real, z0
    else:
...
def compute_weights(N_rep, ref, evecs):
    ...
    temp_weight = np.zeros((np.shape(ref)[1], np.shape(evecs)[1]), dtype = np.complex128)

    if N_replica_space > 1:
        for j in range(N_replica_space):
            temp = np.zeros(N_replica_space)
            temp[j] = 1
            temp_weight += np.kron(temp, np.transpose(np.conjugate(ref))) @ evecs # how much row-th ref is in column-th eigv
    
    # for the static spectrum
    else:
        temp_weight = np.transpose(np.conjugate(ref)) @ np.asarray(evecs)  # how much row-th ref is in column-th eigv

    weight = np.real(temp_weight*np.conjugate(temp_weight))
    
    renormalization = np.real(np.sum(weight, axis=0))
